...
export function registerNetworkTools(server: McpServer, context: ServerContext): void {
  server.registerTool(
    ...
    {
      title: "Get Network Request Aggregates",
      description: "Returns aggregated network request performance and failure data for a Bugfender app.",
      inputSchema: {
  ...
  server.registerTool(
    "get_network_details",
    {
      ...
      description: "Returns metrics and request samples for a domain and endpoint observed by a Bugfender app.",
      inputSchema: {
        app_id: z.string().describe("The public app ID (e.g. 5X3c4veRGV) from list_apps"),
